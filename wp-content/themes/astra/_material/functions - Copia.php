...
function create_post_types()
{
    ...
    $labels = array(
        ...
        "add_new_item" => __("Nova Novidade", "post-type-novidades"), // Texto do botão no alto da página
...
function create_taxonomys()
{
    ...
    $args = [
        ...
        "show_in_menu" => true,
        "query_var" => true,
        "rewrite" => ['slug' => 'cargo', 'with_front' => false],
        "show_admin_column" => true, // Exibe coluna no admin
        "show_in_rest" => true, // Permite uso no editor de blocos
        "rest_base" => "cargo",
    ];
...
// 	$labels->name = 'Publicações';
// 	$labels->singular_name = 'Publicações';
// 	$labels->add_new = 'Adicionar';
// 	$labels->add_new_item = 'Adicionar';
// 	$labels->edit_item = 'Editar';
// 	$labels->new_item = 'Publicações';
...
// 	$labels->menu_name = 'Publicações';
// 	$labels->name_admin_bar = 'Publicações';
// }
...
function adicionar_link_dinamico_menu()
{
    // Defina o ID do post dinamicamente
    $post_id = 38; // Altere esse ID conforme necessário ou obtenha-o de outra forma
    // Cria a URL de edição do post dinamicamente
    $edit_link = admin_url("post.php?post=$post_id&action=edit");
    // Adiciona o item ao menu do admin com a URL desejada
    add_menu_page(
        'Edição de Conteúdos',  // Título da página
        'Conteúdo',            // Nome no menu
        'edit_posts',           // Capacidade necessária
        $edit_link,             // Link direto para a edição do post
        '',                     // Função vazia, pois redirecionaremos para o link
        'dashicons-edit',       // Ícone do menu
        2                       // Posição no menu
    );
}
add_action('admin_menu', 'adicionar_link_dinamico_menu');
